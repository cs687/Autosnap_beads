function Intensityheatmap_final_auto_2(fname,colors,fname_title)
...
[Image_height, Image_width ] = size(var); %Final image dimensions
 ...
 var = eval(sprintf([AA{length(AA)-3}]));
...
%fig_hist=analysesnaphist_auto_beads(fname,colors,fname_title);
title('Beads');xlabel('X (pixels)');ylabel('Y (pixels)');
axis([0,Image_width,0,Image_height]);

print(fig,'Heat-map','-dpng');
saveas(gcf,['Heat-map','.fig']);
